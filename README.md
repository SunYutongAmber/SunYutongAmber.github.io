...
### Final reflections
In this course, I learn how to apply a various machine learning methods to solve social science issues. For example, we learn explanation of the data by using visualization tools. And for explanation, we only need input variable X. We also learn the prediction and classification algorithoms. Classification methods need to output a categorical variable Y , and classify each input X into a category of Y. Typical classification model includes K-Nearest Neighbors and decision tree. Prediction methods require the continuous value of Y variable, and make prediction of each X variable. Typical Prediction model includes linear regression and Logistic regression. Classification and Prediction are supervised machine learning. These algorithoms are frequently used in analyzing social science issue like air pollution prediction using the number of factory in the area, or health situation prediction using age et al. In addition, we also learn how to use the past days' moving average of time series data to predict the future time series data. Typical example is the Cryptocurrency price prediction. We should notice that if a variable A could predict another variable B, that does not necessarily mean A coudl cause B. So we also learn Causal Inference algorithom concept to detect whether one event or variable cause another. Many machine learning models could be used to analyze the causality of some events, and Fast causal inference algorithom is one of them. There are three variables in this situation, X, Y, and Z. Usually we control X varialbe and check whether Z affect Y. Besides, we also learn the optimization mehotd, which means we want to maximize out objective given certain environment by using reinforcement learning algorithom. A typical social problem that could be solved by this method is maximizing the utility of people by selecting from a given bunch of goods. [Reference: Machine Learning for Social Science](https://whimsical.com/machine-learning-for-social-science-match-the-right-tool-to-the--8zuA7Bg5bYQPkRgMJCoywA)

## References

I also learn how to use Github and create my own website. This is really helpful and useful for future research and study.
