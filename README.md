...
- **Application Scenario:** This project uses a [Kaggle dataset](https://www.kaggle.com/code/baotramduong/generate-nft-cryptopunks-with-dggan), and this data set is drawn from the Cryptopunks Transaction board using API (DUON 2021). The date of the data set is from 2017 July to 2021 September. The original dataset contains information about the transaction date, the transaction address, the price of Cryptopunks, the gender, and other attributes of the Cryptopunks’ image. I use the percentage change of Cryptopunk price to get the return on investment value, which is the Y variable. And gender and skin tone are X variables. In Figure 3 in the Splotlight section, I visualize the ROI time series data with the x-axis being time and the y-axis ROI values.

- **Methods:**
  - This research uses Ridge Regression to predict the ROI with gender and skin tone factors. Ridge regression is a method for estimating the coefficient of a multiple regression model. Compared to linear regression, this method improves the coefficient estimation accuracy and assigns different weights to the features (Hoerl and Kennard 1970). The input X variables are gender and skin tone type (value be 0 or 1), and the output Y variable is the return on investment (ROI) (in ETH unit). Using the regression formula with estimated coefficients from Ridge regression, we could conduct prediction.
...
- **Expected Results:** I expect that predict ROI should be close to the real ROI. As showing in Figure 4 in the Splotlight section, the histogram of predicted value and true value are put in the same figure for comparison. The x-axis is the ROI value, and the y-axis is the number of counts. So the overall figure shows the distribution of the predicted ROI (in green bar) and real ROI (in blue bar). The more overlapped part of the two ROI data, the more accurate our prediction is. The research results could be used as reference for investors for picking Cryptopunks according to the gender and skin tone attributes. 
...
![SunYutongAmber.github.io](./spotlight/a.png) 
...
### Final reflections
In this course, I learn how to apply various machine-learning methods to solve social science issues. For example, we learn an explanation of the data by using visualization tools. And for an explanation, we only need input variable X. We also learn the prediction and classification algorithms. Classification methods need to output a categorical variable Y, and classify each input X into a category of Y. Typical classification model includes K-Nearest Neighbors and a decision tree. Prediction methods require the continuous value of the Y variable and making predictions of each X variable. Typical Prediction model includes linear regression and Logistic regression. Classification and Prediction are supervised machine learning. These algorithms are frequently used in analyzing social science issues like air pollution prediction using the number of the factory in the area, or health situation prediction using age et al. In addition, we also learn how to use the past days' moving average of time series data to predict the future time series data. A typical example is the Cryptocurrency price prediction. We should notice that if variable A could predict another variable B, that does not necessarily mean A could cause B. So we also learn the Causal Inference algorithms concept to detect whether one event or variable causes another. Many machine learning models could be used to analyze the causality of some events, and Fast causal inference algorithms are one of them. There are three variables in this situation, X, Y, and Z. Usually we control the X variable and check whether Z affects Y. Besides, we also learn the optimization method, which means we want to maximize our objective function given a certain environment by using reinforcement learning algorithms. A typical social problem that could be solved by this method is maximizing the utility of people by selecting from a given bunch of goods. [Reference: Machine Learning for Social Science](https://whimsical.com/machine-learning-for-social-science-match-the-right-tool-to-the--8zuA7Bg5bYQPkRgMJCoywA)

I also learn how to use GitHub and create my own website. This is helpful and useful for future research and study.

Another important topic we discuss is the ethical issue of AI. Now Computer Programming and algorithms are more and more complicated, and for machine learning, those algorithms are like a black box, as Zeynep mentioned in the TED discussion, we don’t know what happens inside the algorithms (Machine Intelligence Makes Human Morals More Important, Zeynep Tufekci 2016). Someone may think using machine learning could avoid human biases, but this may not be true.  Because the data we used to feed the machine learning model is generated by human beings. If the data is generated from biased people, then the output of the machine learning model may also be biased. If we totally rely on AI for decision-making, we may end up with a biased decision. We could solve this problem by checking the training data set before doing sampling and machine learning. In the future, I plan to contribute to "machine learning for social science" by designing unbiased machine learning models and selecting the training data set cautiously to avoid biased data.
